import random
from lib.battleroyale import get_real_room, battle_royale_information
...
def get_kill_room(number_of_games, room_id):
    """
    获取指定房间在指定场次的击杀次数
    """
    df = read_csv()
    sorted_df = df.sort_values(by="期数", ascending=False)
    latest_df = sorted_df.head(number_of_games)
    count = 0
...
def get_room_rate(number_of_games, room_id):
    """
    获取指定房间的击杀率
    :param number_of_games: 最近场数
    :param room_id: 房间编号
    :return: 击杀率
    """
    return get_kill_room(number_of_games, room_id) / number_of_games
